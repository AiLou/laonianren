...
import com.wu.service.BillpingServiceImpl;
...
public class BillController {
    ...
    @Autowired
    private BillpingServiceImpl billpingService;
    ...
    @RequestMapping("/addbillping")
    public String addBillping(Bill bill, HttpSession session) {
        bill.setProductCount(new BigDecimal(String.valueOf(bill.getProductCount())).setScale(2, BigDecimal.ROUND_DOWN));
        bill.setTotalPrice(new BigDecimal(String.valueOf(bill.getTotalPrice())).setScale(2, BigDecimal.ROUND_DOWN));
        bill.setCreatedBy(((User) session.getAttribute(Constants.USER_SESSION)).getId());
        bill.setCreationDate(new Date());
        int i = billpingService.addBill(bill);
        if (i > 0) {
            return "redirect:/jsp/billpingManagement";
        } else {
            return "WEN-INF/jsp/billpingadd";
        }
    }
}
